...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        '''
        Inorder to find error, I sumed up all the products of weight and the difference of label and 1 
        when featureVal was smaller than 0, or the products of weight and label when that was bigger than 0.

        And I appended all the result error into list "err", then found out which was the minimum, and finally returned
        the value of the minimum, and returned function "WeakClassifier" with parameter of a row in list "features"
        with the index of the minimun.
        '''
        err=[]
        ...
        for i in range(len(featureVals)):
            ...
            for j in range(len(featureVals[i])):
                if featureVals[i][j] < 0:
                    arg = arg + abs(labels[j] - 1) * weights[j]
                else:
                    arg = arg + abs(labels[j]) * weights[j]
                
            err.append(arg)
                
        bestError = min(err)
        bestClf = err.index(min(err))   

        # raise NotImplementedError("To be implemented")
        # End your code (Part 2)
        return WeakClassifier(features[bestClf]), bestError
