...
class Agent():
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if done:
            ...
            nextBest = np.max(self.qtable[next_state])
        self.qtable[state][action] = (1 - self.learning_rate) * self.qtable[state][action] + self.learning_rate * (reward + self.gamma * nextBest)
