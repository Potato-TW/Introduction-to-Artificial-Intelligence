...
class Agent():
    ...
    def learn(self):
        ...
        state, action, reward, nextstate, done = self.buffer.sample(self.batch_size)
        
        state = self.evaluate_net(torch.FloatTensor(state))
        reward = torch.FloatTensor(reward)
        nextstate = self.target_net(torch.FloatTensor(nextstate))

        loss = torch.tensor(0., dtype=torch.float32)
        
        for i in range(self.batch_size):
            loss += (state[i][action[i]] - (reward[i] + self.gamma * (1-done[i]) * torch.max(nextstate[i])))**2
